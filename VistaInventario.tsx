        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col shrink-0">
            <div className={`px-6 py-4 border-b flex flex-col gap-1 ${colorClass}`}>
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-black text-business-orange bg-white/70 px-2 py-0.5 rounded-full shadow-sm">{lista.length}</span>
                </div>
                <p className="text-sm text-business-olive font-medium">{subtitulo}</p>
            </div>
            <div className="overflow-x-auto inner-scroll">
                <table className="w-full text-left min-w-[1000px]">
                    <thead className="bg-slate-50 text-slate-600 font-black uppercase tracking-widest text-xs sticky top-0 z-10 border-b">
                        <tr>
                            ...
                            return (
                                <tr key={i.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-black text-slate-800 text-[13px] leading-tight flex items-center gap-2">
                                                    {i.nombre || 'Sin Nombre'}
                                                    {i.source === 'EXTERNA' && <span className="text-sm bg-sky-100 text-sky-700 px-1.5 py-0.5 rounded uppercase tracking-wider font-bold">NetSuite</span>}
                                                    {i.estado === EstadoInsumo.COMPLETADO && i.registrado_netsuite && (
                                                        <span className="text-sm bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded uppercase tracking-wider font-bold flex items-center gap-0.5" title="Registrado en NetSuite">
                                                            <ShieldCheck size={10} /> NS {i.codigo_netsuite}
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="text-xs text-slate-600 font-bold uppercase mt-0.5 flex items-center gap-1.5">
                                    ...
                                    <td className="px-6 py-3">
                                        <div className="space-y-1.5 max-w-[180px]">
                                            <div className="flex justify-between items-center px-0.5">
                                                <span className={`text-sm font-black uppercase px-2 py-0.5 rounded-full border shadow-sm ${ESTILOS_ESTADO_INSUMO[i.estado]}`}>
                                                    {ETIQUETAS_ESTADO_INSUMO[i.estado]}
                                                </span>
                                                <span className="text-xs font-black text-business-orange">{prog}%</span>
                                            </div>
                                    ...
                                    <td className="px-6 py-3 text-right">
                                        ...
                                        <div className="text-sm font-black text-slate-600 uppercase">x{i.cantidadCompra} {i.unidad}</div>
                                    </td>
                                    <td className="px-6 py-3 text-right">
                                        <div className="font-black text-slate-900 text-sm tracking-tight">
                                            ...
                                            <span className="text-xs text-slate-600 ml-0.5 font-bold">/{i.unidadConsumo || 'g'}</span>
                                        </div>
                                    </td>
        ...
        <div className="space-y-4 animate-in fade-in duration-500 h-full flex flex-col">
            <header className="flex flex-col md:flex-row justify-between items-end gap-4 shrink-0">
                <div>
                    ...
                    <p className="text-slate-700 font-medium italic text-sm mt-1">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="relative md:w-40">
                        ...
                        <select
                            ...
                            className="w-full pl-9 pr-6 py-2 bg-white border border-slate-200 rounded-xl appearance-none font-bold text-sm text-slate-600 focus:ring-4 focus:ring-business-mustard/20 outline-none shadow-sm cursor-pointer"
                        >
                    ...
                    {(role === 'CHEF' || role === 'COMPRAS' || role === 'ADMIN') && (
                        <button
                            ...
                            className="flex items-center gap-1.5 bg-business-orange text-white px-4 py-2 rounded-xl font-black uppercase text-sm shadow-lg hover:bg-business-orange/90 transition-all active:scale-95"
                        >
            ...
            <div className="flex border-b border-slate-200 mt-2 px-6 gap-8 bg-white shrink-0">
                <button
                    ...
                    className={`py-3 text-sm font-black uppercase tracking-wider border-b-2 transition-all ${tabActiva === 'EXTERNA' ? 'border-sky-500 text-sky-600' : 'border-transparent text-slate-600 hover:text-slate-600'}`}
                >
                    <div className="flex items-center gap-2">
                        Insumos Externos (NetSuite)
                        <span className={`px-2 py-0.5 rounded-full text-xs ${tabActiva === 'EXTERNA' ? 'bg-sky-100 text-sky-700' : 'bg-slate-100 text-slate-700'}`}>
                ...
                <button
                    ...
                    className={`py-3 text-sm font-black uppercase tracking-wider border-b-2 transition-all ${tabActiva === 'INTERNA' ? 'border-business-orange text-business-orange' : 'border-transparent text-slate-600 hover:text-slate-600'}`}
                >
                    <div className="flex items-center gap-2">
                        Insumos Nuevos
                        <span className={`px-2 py-0.5 rounded-full text-xs ${tabActiva === 'INTERNA' ? 'bg-business-mustard/20 text-business-orange' : 'bg-slate-100 text-slate-700'}`}>
                ...
                {tabActiva === 'INTERNA' && (
                    <>
                        {insumosInternos.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center p-12 text-slate-600 bg-white rounded-3xl border border-slate-100 shadow-sm mt-4">
...
function EditorInsumo({ insumo, onClose, onSave, role, fasesConfig, proveedores }: { insumo: Insumo, onClose: () => void, onSave: (i: Insumo) => void, role: Rol, fasesConfig: FaseFluxoInsumo[], proveedores: any[] }) {
    ...
    const renderCampo = (campo: keyof Insumo) => {
        ...
        if (campo === 'locales') {
            return (
                <div key={campo} className="space-y-1">
                    <label className="text-xs font-black text-slate-600 uppercase">CREAR EN LOCALES:</label>
                    <select
                        value={datos[campo] as string || 'No'}
                        onChange={e => handleFieldChange(campo, e.target.value)}
                        disabled={!esEditableActual}
                        className="w-full p-2.5 bg-white border border-slate-200 rounded-xl font-bold text-slate-700 text-xs outline-none focus:ring-4 focus:ring-business-mustard/10 transition-all disabled:bg-slate-50 disabled:text-slate-600"
                    >
        ...
        if (typeof datos[campo] === 'boolean' || campo === 'lote' || campo === 'alergenos') {
            return (
                <div key={campo} className="flex items-center gap-2 p-3 bg-business-beige/30 rounded-xl border border-business-mustard/10">
                    ...
                    <label className="text-sm font-black text-business-olive uppercase">{label}</label>
                </div>
            );
        }
        ...
        if (campo === 'documentos') {
            ...
            return (
                <div key={campo} className="col-span-full space-y-3">
                    <label className="text-xs font-black text-slate-600 border-b pb-1 w-full block uppercase mb-1.5">{label}</label>
                                ...
                                <div className="flex items-center gap-3 overflow-hidden">
                                    ...
                                    <div className="min-w-0 pr-12">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <a href={`${doc.url}`} target="_blank" rel="noopener noreferrer" className="text-xs font-black uppercase tracking-wider text-business-orange hover:text-business-olive flex items-center gap-1">
                    ...
                    {esEditableActual && (
                        <div className="relative">
                            ...
                            <div className={`p-6 border-2 border-dashed ${subiendoArchivo ? 'border-indigo-300 bg-indigo-50' : 'border-slate-300 hover:border-indigo-400 hover:bg-indigo-50/30'} rounded-2xl flex flex-col items-center justify-center text-slate-700 gap-2 transition-all`}>
                                <Upload className={`w-8 h-8 ${subiendoArchivo ? 'text-indigo-400 animate-bounce' : 'text-slate-600'}`} />
                                <span className="text-xs font-bold">{subiendoArchivo ? 'Subiendo archivo...' : 'Haz clic o arrastra un archivo aquí'}</span>
                                <span className="text-xs text-slate-600 font-medium">Soporta PDF, Word, Excel, Excel y más.</span>
                            </div>
                        </div>
                    )}
                </div>
        ...
        if (['unidad', 'unidadStock', 'unidadConsumo', 'tipoMaterial', 'tipoImpuesto', 'tipoRotacion'].includes(campo)) {
            ...
            return (
                <div key={campo} className="space-y-1">
                    <label className="text-xs font-black text-slate-600 uppercase">{label}</label>
                    <select
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-5xl rounded-[2rem] shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
                <div className="p-5 border-b bg-slate-50/50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs font-bold text-business-orange uppercase mt-0.5">ID: {datos.id}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 text-slate-600 hover:text-rose-500 rounded-xl transition-all"><X className="w-5 h-5" /></button>
                </div>

                <div className="flex border-b overflow-x-auto bg-white px-2">
                    {fasesActivas.map((f, idx) => (
                        <button key={f.id} onClick={() => setFaseActivaId(f.id)} className={`px-5 py-3 flex items-center gap-2 text-xs font-black uppercase tracking-widest border-b-4 transition-all shrink-0 ${faseActivaId === f.id ? 'border-business-orange text-business-orange bg-business-mustard/5' : 'border-transparent text-slate-600'}`}>
                            <div className={`w-5 h-5 rounded-md flex items-center justify-center text-sm ${faseActivaId === f.id ? 'bg-business-orange text-white' : 'bg-slate-200 text-slate-700'}`}>{idx + 1}</div>
                            {f.nombre}
                        </button>
                    ))}
                </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-2 duration-300" key={faseActual.id}>
                            <div className="flex items-center gap-3 p-4 bg-business-beige/20 rounded-2xl border border-business-mustard/10">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-business-olive">{esEditableActual ? "Permisos activos." : `Sólo ${faseActual.rolResponsable} o ADMIN.`}</p>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-2 duration-300 mt-6 pt-6 border-t border-slate-100">
                            <div className="flex items-center gap-3 p-4 bg-sky-50/50 rounded-2xl border border-sky-100">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-sky-700">Registrar código ERP (Exclusivo para COSTOS)</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                                <div className="space-y-1">
                                    <label className="text-xs font-black text-slate-600 uppercase">CÓDIGO NETSUITE</label>
                                    <input 
                                        type="text" 
                                        ...
                                        className="w-full p-2.5 bg-white border border-slate-200 rounded-xl font-bold text-slate-700 text-xs outline-none focus:ring-4 focus:ring-sky-500/10 transition-all disabled:bg-slate-50 disabled:text-slate-600" 
                                    />
                                </div>
                                <div>
                                    {datos.registrado_netsuite ? (
                                        ...
                                        <button 
                                            ...
                                            className="w-full p-2.5 bg-sky-600 text-white font-black uppercase text-sm rounded-xl shadow-md hover:bg-sky-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 h-full"
                                        >
                ...
                <div className="p-6 border-t bg-slate-50/80 flex justify-end gap-3 rounded-b-[2rem]">
                    <button onClick={onClose} className="px-6 py-3 text-business-olive font-black uppercase text-sm rounded-xl hover:bg-business-beige transition-all">Descartar</button>
                    
                    {datos.estado !== EstadoInsumo.COMPLETADO && (
                        <button onClick={() => onSave(datos)} className="px-8 py-3 bg-slate-200 text-slate-700 font-black uppercase text-sm rounded-xl hover:bg-slate-300 transition-all flex items-center gap-2">
                    ...
                    {datos.estado === EstadoInsumo.PENDIENTE_COMPRAS && (role === 'COMPRAS' || role === 'ADMIN' || role === 'CHEF') && (
                        <button 
                            ...
                            className="px-8 py-3 bg-business-orange text-white font-black uppercase text-sm rounded-xl shadow-lg hover:bg-business-orange/90 transition-all flex items-center gap-2"
                        >
                    ...
                    {datos.estado === EstadoInsumo.PENDIENTE_LOGISTICA && (role === 'LOGISTICA' || role === 'ADMIN') && (
                        <button 
                            ...
                            className="px-8 py-3 bg-emerald-600 text-white font-black uppercase text-sm rounded-xl shadow-lg hover:bg-emerald-700 transition-all flex items-center gap-2"
                        >
                    ...
                    {datos.estado === EstadoInsumo.COMPLETADO && (
                        <button onClick={() => onSave(datos)} className="px-8 py-3 bg-emerald-600 text-white font-black uppercase text-sm rounded-xl shadow-lg hover:bg-emerald-700 transition-all flex items-center gap-2">
